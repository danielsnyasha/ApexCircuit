  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "bg-[#0D0D14]/90 backdrop-blur-xl border-b border-white/5 shadow-2xl"
          : "bg-transparent"
      }`}
    >
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <Show when="signed-in">
              <a
                href="/admin"
                className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/10 text-gray-400 hover:text-white hover:border-white/20 text-xs font-medium transition-all duration-200"
              >
            ...
            <Show when="signed-out">
              <SignInButton mode="redirect">
                <button className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/10 text-gray-400 hover:text-white hover:border-white/20 text-xs font-medium transition-all duration-200">
                  Sign In
                </button>
              </SignInButton>
            </Show>
            ...
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              ...
              <SheetContent
                side="right"
                className="w-72 bg-[#0D0D14] border-white/10 p-0"
              >
                <div className="flex flex-col h-full p-6 pt-12">
                  ...
                  <nav className="flex flex-col gap-1">
                    {navLinks.map((link) => (
                      <button
                        ...
                        className="text-left px-4 py-3 text-gray-300 hover:text-white hover:bg-white/5 rounded-lg transition-all duration-200 text-sm font-medium"
                      >
                        {link.label}
                      </button>
                    ))}
                  ...
                  <div className="mt-auto flex flex-col gap-2">
                    <Show when="signed-out">
                      <SignInButton mode="redirect">
                        <button className="w-full px-4 py-3 rounded-lg border border-white/10 text-gray-300 text-sm font-medium hover:text-white hover:border-white/20 transition-all duration-200">
                          Sign In
                        </button>
                      </SignInButton>
                    </Show>
                    <Show when="signed-in">
                      <a
                        href="/admin"
                        className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-white/10 text-gray-300 text-sm font-medium hover:text-white transition-all duration-200"
                      >
